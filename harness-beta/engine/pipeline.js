...
req = req || {}
if (!req.request) throw new Error('harness-beta-engine: args.request (raw request string) is required')
const routeSelection = req.route_selection || { route: 'dynamic_workflow', reason: 'direct beta Workflow invocation' }
...
function codexAdapterPrelude(dir) {
  const explicit = codexAdapterPath
    ...
    : ''
  return (
    mountSkill('harness-beta:codex-control', 'resolve the Codex CLI adapter for the beta plugin') +
    `\n1. Use Bash to create ${dir}/.\n` +
    explicit +
    `${explicit ? '3' : '2'}. If ADAPTER is still unset, locate it with Bash: prefer ` +
    `harness-beta/engine/codex-exec-adapter.mjs; if absent try ` +
    `harness/engine/codex-exec-adapter.mjs or .claude/harness/engine/codex-exec-adapter.mjs; ` +
    `if absent follow the codex-control ` +
