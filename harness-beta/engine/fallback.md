# Agent Team engine — harness beta selected route

The alternative engine (`pipeline.js`) uses the **Dynamic Workflow (DW) runtime** (`agent()`,
`parallel()`, `pipeline()`). This file is the beta's **Agent Team route**: the same six stages
and contract, executed by a
...
**Selection rule (hard):** use this when the `harness-beta:harness` selector chose
`agent_team`, or when Workflow is unavailable and role-separated agents are available. A
successful beta selection may choose this route even when Workflow exists. Never run both paths
for one request and never switch paths merely because a gate rejected the work.
...
```
.harness-run/<slug>/
  manifest.json              # {request, route_selection, context?, max_retries, team:{lead,roles[]}, subgoals:[{id,wave}], stages:{...}}
...
1. Create `RUN=.harness-run/<slug>/` and write `manifest.json` with the request, the selector's
   `route_selection`, `max_retries` (default 2), empty `subgoals`/`stages`, and the declared
   Agent Team roles/ownership.
2. Detect external providers — **only when the run was explicitly started with
   ...
   `harness-beta:codex-control` and resolve `codex-exec-adapter.mjs` from an explicit path,
   `harness-beta/engine/`, `harness/engine/`, `.claude/harness/engine/`, or the plugin root named in the project's
...
Record the subgoal ids + dependency wave (next section) into `manifest.json`.

## 3. Order the subgoals into dependency waves

Topologically sort `subgoals` by `deps`. Dependency-ready subgoals may share a wave only when
their claimed file ownership and checks are disjoint; otherwise place them in successive waves.
Run one wave at a time and cap concurrent subgoals at three. On an unsatisfiable cycle, record
the cycle and stop; do not invent an unsafe order.

Add one `TaskCreate`-style checklist item per subgoal. Only the current wave may be
`in_progress`; mark each item `completed` only after its QualityGate passes or retries exhaust,
then start the next wave.

## 4. Per subgoal — Implement → Test → QualityGate, looped (bounded by max_retries)

For each subgoal `<id>` in the current wave, run its independent loop from attempt `n = 1` up to
`max_retries` (default 2). Never run two nodes concurrently if either can touch the same file or
uses a shared/global check:
...
Run `node harness-beta/engine/fallback-check.mjs RUN` (or the equivalent plugin-root path). It deterministically verifies every required
...
- Control flow is orchestrated by you, not a deterministic script. Dependency waves and the
  three-subgoal cap constrain concurrency, but they are less reproducible than `pipeline.js`.
