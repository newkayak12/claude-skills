...
py(){ find $T -name '*.py' -o -name '*.kt' -o -name '*.hurl' -o -name '*.ts' -o -name '*.js' -o -name '*.sh' | grep -v __pycache__; }
...
c cleanup     $([ $(py | xargs grep -n 'finally\|trap .*EXIT\|defer' 2>/dev/null | wc -l) -ge 1 ] && echo 1 || echo 0)
c namespace   $([ $(py | xargs grep -n 'uuid\|time()\|run_id\|RUN_ID\|randint\|date +%s' 2>/dev/null | wc -l) -ge 1 ] && echo 1 || echo 0)
