---
name: mcp-builder
description: 'Use when someone wants to expose an external service or API as a tool an LLM can call — building an MCP server from scratch, designing tool schemas, handling auth, or structuring resources and prompts. Applies to both Python (FastMCP) and TypeScript (MCP SDK) implementations. For CLI wrappers around such tools, consider cli-developer.'
