...
33개 중 5개는 **워크플로 진입점**입니다. 직접 일하지 않고 전문 스킬들을 정해진 순서로 구동하며,
중간 단계부터 합류할 수 있습니다.
...
| 실행 중인 서버에 다단계 API 흐름 테스트 수립·실행 | `api-scenario-test` |
...
이메일 유니크 제약을 심은 픽스처 주문 API로 측정했습니다(`evals/`): 무스킬 3런은 모두 2/7 —
고정 이메일, 호스트 하드코딩, 1회 실행, 스펙 없음 — 이고 두 번째 실행에서 409로 깨지는 걸
스스로 모릅니다. 스킬 3런은 7/7, 6/7, 7/7이고 감점은 채점기가 Hurl 문법을 못 읽은 것입니다.
