...
- [ ] **phase-guard 후속** (#013b 인계) — ⓐ 전환 무결성: current_phase 직접편집 우회(metrics.json은 session-counter가 갱신해 보호 불가)를 닫기 — 별도 보호파일 분리 또는 phase-guard가 current_phase 변경 Edit 감시. ⓑ 실설치 시운전: phase-guard가 실제 발화·차단하는지 확인(#012 시운전 패턴 — 이 레포엔 미설치라 메타-dogfood 미발화). ⓒ feedback report: `.claude/.feedback/feedback.jsonl` 누적분을 beta report로 정리하는 흐름.
