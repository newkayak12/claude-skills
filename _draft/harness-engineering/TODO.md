...
마지막 갱신: 2026-06-08 (**v0.3.3 — update 경로 결함 수정 (N=0 첫 실측 수확)**: marketplace update 후 `harness:install` 이 "이미 설치됨"으로 막혀 새 버전이 프로젝트에 안 닿던 결함(ⓗ). install SKILL 재배선(Step A 재-벤더 항상 실행, user-rules 존재는 그 단계만 skip) + project-install 버전 인식 마커(`{version,vendored_at}`, 신규/최신/업그레이드 정보성 보고, 구형 하위호환). test 8 추가. 13/13 green. **dogfood 로는 안 보이던 update UX — N=0 의 가치 첫 입증**. / 직전 **v0.3.2 — P0 잔여 닫힘: force-close 감사성(ⓔ) + accept-new-baseline(ⓖ) + GOAL delivery 명문화(ⓒ)**: close `--force` 가 `--adr` 결박+blackbox `force-close` 기록(게이트 우회=흔적, phase-advance 대칭), `bar-register --baseline-reset` 1급 선언으로 정당한 ratchet baseline 상향 표현(force 아닌 명시·리뷰·체인 게이트, F3 해소), GOAL §2.1 신설로 "install=프로젝트 .claude/ scaffold=ambient governance" 박음(드리프트 재발 방지). 소스 13/13·generated 9/9 green, drift 0, 3버전 동기. **N=0 여전 잔존**(코드 무결성만 강화, 실프로젝트 미입증). / 직전 **v0.3.1 — Tier1 게이트 무결성 + harness:plan 정렬**: H1 phase 신뢰앵커 metrics→tamper-evident `phase.jsonl` chain, H2 confirm `--confirmation-note` 강제, harness:plan 유령참조 제거+산출물계약 phase_gates evidence 일원화. / 직전 **#015 deploy-kill-check-retire 종료** — 첫 순수 "빼기" 사이클. deploy-kill-check.py(발화0·효과최약)+kill-check.py(고아 엔진) 은퇴 → mechanism-count **28→26**, ratchet floor **27→26** 단조개선으로 **ADR-0001 standing 부채 해소**(--force 불요, ADR-0001 예고 "27 복원"을 26으로 초과달성). metrics `kill_check` 필드 제거(vestigial). 히스토리(devils/GOLDEN) 미변경·라이브 문서만 갱신. 독립 리뷰어 R1~R3 pass(count 직접 재산출+find_regressions 직접 실행). 잔여 orphan: session_count(소비자 소멸, rank4와 묶어 후속). / 직전 #014b: installer F5 프로덕션 수정. #014: P0 delivery ⓐ scaffold+ADR-0001)
...
- [ ] **install = 프로젝트 `.claude/` scaffold = ambient governance** — 현재는 전역 플러그인 + 사용자-전역 `~/.harness/user-rules.md` + *명시적* `harness:cycle` 호출에 갇혀, (1) per-project 타겟팅 불가(전역 한 버전), (2) "그냥 쓰면 그 레포에선 자동으로 하네스 아래서 AI 동작"이 안 됨(사이클이 opt-in). **올바른 모델(revfactory/harness 패턴 + 이 레포 자신이 그렇게 동작): install이 대상 프로젝트 `.claude/`에 hooks(settings.json 등록)+CLAUDE.md(사이클 규율 박기)를 scaffold → `.claude/` 자동로드로 타겟팅+ambient 동시 해결.** `cycles/`는 이미 프로젝트 로컬. 전역 플러그인은 *설치기/생성기*로 축소. **재배선이지 재작성 아님 — hooks·게이트·ratchet 페이로드는 그대로.**
  - [x] **ⓐ delivery 재설계** (#014 종료 + **#014b 프로덕션 수정 종료**) — `project-install.py`가 대상 프로젝트 `.claude/`에 페이로드 vendoring + `settings.json`(hooks→`$CLAUDE_PROJECT_DIR/.claude/harness`) + `CLAUDE.md`(governance) scaffold. 멱등·기존 보존. install SKILL Step A 추가. 벤더링 hook은 상대 fallback으로 CLAUDE_PLUGIN_ROOT 없이 자급. **#014b: #014의 F5(installer가 harness-export subprocess 의존→프로덕션 file-not-found) 교정 — installer 직접 재귀복사로 자급화 + 글로벌 플러그인에 installer 포함 + 프로덕션 경로 테스트(빌드된 harness/서 벤더링, dogfood-cheat 차단). 독립 리뷰어 R1~R3 pass + 자체 프로덕션 프로브. clean close.**
  - [ ] **ⓑ ceremony-free 사이클** — 프로젝트 CLAUDE.md/hook이 AI를 디폴트로 사이클 규율에 묶기. "의식 없이 자동" 의 마지막 한 칸. (scaffold된 CLAUDE.md governance가 1차 — 실제 AI 행동 입증은 post-delivery N=0)
  - [x] **ⓒ GOAL.md 명시화** (v0.3.2) — "install = 프로젝트 `.claude/` scaffold = ambient governance"를 GOAL **§2.1 신설**(+ §2 step[2]·§3.6 제약·§6 changelog)에 박아 같은 드리프트 재발 방지. 역할분담(강제=hook/안내=ambient CLAUDE.md/스킬=진입점) 명문화.
  - [x] **ⓓ 버전 bump → 0.3.0** (delivery 모델 완성 마일스톤) — marketplace.json + plugin.json×2 동기(0.2.0→0.3.0). **1.0.0 아님**: 북극성(게이트가 품질저하 막음)이 N=0 미입증이라 pre-1.0 유지(정직 ethos). 시운전(rank10 실측)이 효과 입증하면 그때 1.0 재검토. 부수효과: marketplace staleness 함정 해소(버전 동일이라 update가 새 커밋 안 당기던 문제).
  - [x] **ⓔ close `--force` blackbox 기록(F2, high)** (v0.3.2) — `close-cycle.py --force`가 이제 `--adr <존재파일>` 결박(없으면 exit 2 거부) + blackbox에 `{kind:"force-close", adr, regressions(record-only find_regressions), ts}` append. phase-advance --force 와 대칭으로 게이트 우회가 감사 흔적에 가시화. test-close-cycle D(--adr 누락 차단)·E(기록 확인) 추가. **게이트 우회 기록은 게이트만큼 중요 — 닫힘.**
  ...
  - [x] **ⓖ ratchetlib accept-new-baseline (F3, #014/#015 인계)** (v0.3.2) — `bar-register --baseline-reset` 신설: 그 축 바가 이전 watermark 를 *대체*(force 아닌 **1급 선언** — 명시 플래그 + immutable bar.jsonl + pass 리뷰가 게이트). `compute_floor` 는 closed_at 시간순 처리로 reset 이 watermark 대체, `find_regressions` 는 reset 축을 회귀로 오판 안 함, floor 출력에 `[baseline-reset]` 마커. 빼기 불가능한 정당 +1(예: mechanism-count) 표현 가능. test-ratchet-check 11~14(reset 통과·floor 대체·이후 회귀 여전 차단). **force(ⓔ)와 병존 — force=미선언 탈출구(blackbox), reset=선언·리뷰 baseline. 닫힘.**

  - [x] **ⓗ update 경로 결함 (N=0 첫 실측, v0.3.3)** — **실사용 발견**: marketplace `update` 후 `harness:install` 이 "이미 설치됨"으로 거부/종료 → 새 버전이 프로젝트에 *안 닿음*. 원인 3겹: (1) user-rules-init 존재 시 install SKILL Step1 "STOP"이 온보딩 전체를 멈춤, (2) 벤더 마커에 버전 없어 업그레이드 감지 불가, (3) SKILL 전체가 "첫 실행" 프레임(update 모드 부재). **수정**: A. install SKILL 재배선 — Step A(재-벤더)는 첫설치/update *항상* 실행, user-rules 존재는 *그 단계만* skip(전체 STOP 금지). B. project-install 버전 인식 — 마커를 `{version, vendored_at}` JSON 으로, 재실행 시 `신규설치/이미최신/업그레이드 vX→vY` *정보성* 보고(거부 아님), 구형 plain 마커 하위호환. test-project-install 8(신규/마커기록/이미최신/구형마커/업그레이드). **N=0 가 첫 결함을 내놓음 — dogfood 로는 안 보이던 update UX.**
...
- [ ] **phase-guard 후속** (#013b 인계) — [x] **ⓐ 전환 무결성 닫힘(Tier1 H1, 0.3.1)**: phase-guard가 metrics.json이 아니라 tamper-evident `phase.jsonl` hash-chain에서 phase·게이트 도출. phase-advance가 유일 기록자, immutability가 직접편집(Edit) 차단, verify_chain이 Bash append, "부재→차단"이 삭제 방어. metrics 직접편집 위조로 게이트 우회 불가(test H1-a/b/c 실증). 한계: Bash 유효해시 통째 위조는 위협모델 밖(bars 동일). + **H2**: collaborative confirm에 `--confirmation-note` 강제(허니시스템→감사기록). ⓑ 실설치 시운전: phase-guard 실제 발화·차단 확인(이 레포 미설치라 미발화, **N=0 잔존**). ⓒ feedback report: feedback.jsonl 누적분 beta report 정리.
- [x] **harness:plan 불일치 해소 + 산출물 계약 일원화** (0.3.1) — plugin.json/README/work·review SKILL/export/W5의 유령 스킬 `harness:plan` 참조 제거(git 이력상 미구현). work/review가 읽던 유령 파일 `cycles/active/{spec,design,plan}.md` → `metrics.json phase_gates evidence`(정본)로 교체. plan 안내는 cycle Step6(흡수)+project-install CLAUDE.md governance(ambient "plan-before-code=hook 강제")로. 원칙: 강제=hook, 안내=ambient, 스킬=대화형 진입점. test-phase-guard wc 이식성 패치 부수. 16/16 green.
...
- [x] **#013b** phase-guard + feedback-capture — `phase-guard.py`(PreToolUse, current_phase∈{design,planning}+코드확장자→exit2 차단, R-PG01 "No code before design"을 soft 권고에서 물리 게이트로) + `phase-advance.py`(analysis→…→validation 인접전환 강제, --force=blackbox) + `feedbacklib.py`(.claude/.feedback/feedback.jsonl 마찰 자동기록, fail-soft, beta report 원료). 4번째 차단성 Sensor. test 6+4+2 매트릭스 비-vacuous. **실설치 시운전·전환 무결성(직접편집 우회)은 후속**. B1~B5 독립 리뷰 pass. `cycles/20260605-phase-guard-and-feedback-capture/`
