...
| 2 | 2026-06-10 | Shipped v0.3.8 (16 scripts + 7 wired hooks + 95 rules + delivery model, 11 closed cycles) | HA-8..10, CA-8..10, CV-2, PF-8..10 |
...
# Review #2 — 2026-06-10 (Shipped v0.3.8)

**대상**: `harness/` 빌드 산출물 전체 — 16 scripts + 7 *실제 wiring된* hooks + 95 rules(41 base + 54 situational) + delivery model(`project-install.py` vendoring) + 11개 closed cycle.
**단계**: 컨셉 stage를 넘어 *동작하는 설치형 플러그인*으로 성숙. Review #1 이후 #001~#011 사이클 완주.
**Goal 기준**: [`GOAL.md`](./GOAL.md) §1 (install → applied) + §3.5 (첫 사이클 가치 증명).
**이전 review와의 관계**: Review #1의 PF-1(operational layer)·PF-2(hook 구현)·PF-3(3-tier 구조)는 Resolution log대로 *충족*됐다 — spec-only hook이 실제 차단 Sensor로 wiring됐고 CV-1은 "사람→코드"로 *부분* 대응됐다. Review #2는 그 성공을 인정한 위에서 **남은 균열을 재조준**한다: 강제는 실재하게 됐으나(CV-1 완화), *강제된 것이 옳은 것인지*는 여전히 미검증이다. CA-8은 Resolution log가 11회 반복한 "H1(실사용)은 다음 프로젝트에서 측정 예약"을 정면으로 문제삼는다.

## Position

> harness-engineering(v0.3.8)은 솔로 개발자가 계획 단계를 건너뛰지 못하게 하는, 설치 가능한 규율 시스템으로서 올바른 방향으로 성숙하고 있다.

## Steel-man

자기 협상에 무너지지 않는 유일한 강제는 결정론적 훅이다. hard(차단 훅)/soft(주입) 분리는 정확하고, hash chain은 증거 조작을 막고, #015 은퇴는 빼기도 가능함을 증명했다 — 설계의 내적 일관성은 실제로 높고, Review #1의 spec-only 비판(CA-2)을 실제 wiring으로 상당 부분 갚았다.

## 숨은 가정 (HA-8부터)

### HA-8: 프로세스 준수가 더 나은 제품 결과로 이어진다
ratchet·chain·gate 전부가 *준수*를 측정하지만, 준수가 *결과*(더 좋은 제품, 더 빠른 출시, 더 높은 품질)의 대리 지표라는 증거는 없다. compliance ≠ outcome.

### HA-9: 기계적으로 강제할 가치가 있는 것이 "장부의 무결성"이다
7개 wired hook이 지키는 건 전부 북키핑(jsonl 불변·phase 순서·watermark)이다. 품질을 실제로 결정하는 판단(증거 해석·설계 선택)은 강제 대상에서 빠져 있다는 가정.

### HA-10: 다른 솔로 개발자들이 이 수준의 규율을 원한다
marketplace 배포(GOAL §1)는 외부 수요를 전제한다. 그러나 harness 자신의 §3.5("첫 사이클에서 가치 증명")가 외부 사용자에게 검증된 적이 없다 — author=user(CV-1)라 N=0.

## 반론 (CA-8부터)

### CA-8: harness는 자기 자신의 Gate 1을 통과한 적이 없다 `[assumption]` · severity: **critical**

이 시스템의 제1원칙은 "만들기 전에 검증하라"인데, harness 자체는 그 검증을 받은 적이 없다. 11개 사이클·v0.3.8까지 harness가 거버넌스한 유일한 제품은 **harness 자신**이다. Resolution log는 #001부터 일관되게 "H1(실사용)은 다음 프로젝트에서 측정 예약"이라 적었고 — 그 *연기(defer)가 11회 반복되며 패턴이 됐다*. 가설 등록자 = 실행자 = 심판자 = 대상이 전부 동일인이라 사전등록(pre-registration)의 효력이 구조적으로 소거된다(과학에서 사전등록이 작동하는 이유는 심판이 외부에 있기 때문). HA-3(피드백 소스)이 11사이클 내내 채워지지 않은 채 시스템만 커졌다.

**선례**: CMMI류 중량 프로세스 — 프로세스 성숙도 인증과 제품 성공의 상관은 입증되지 않았고, 인증 자체가 목적이 됐다.

### CA-9: 강제의 비대칭 — 훅은 장부를 지키고, 품질은 무방비다 `[structural]` · severity: **high**

Review #1의 CA-4(해시 체인=theater)는 "hook이 spec-only"였기에 절반만 옳았다. 이제 hook은 실제 wiring됐다(CV-1 완화) — 그러나 *그 강제가 향한 곳*이 문제다. hard 강제 4종(phase-guard·hypothesis-immutability·active-symlink-guard·ratchet)이 지키는 건 전부 북키핑이다. 정작 결과를 결정하는 것(인터뷰를 정직하게 해석했는가, 설계가 좋은가)은 전부 soft 주입(모델의 자발적 준수)에 남아 있다. 자기 문서가 이미 인정한다: AP-01(검증 연극)·AP-04(부정 증거 망각)는 **모든 게이트를 통과하면서** 일어난다. SHA256 체인이 막는 *narrative 재구성*(CA-4의 진짜 실패 모드)은 체인을 건드리지도 않고 게이트를 지난다. 강제가 가장 강한 곳과 위험이 가장 큰 곳이 어긋나 있다.

**선례**: SOX/감사 추적 — 감사 로그 무결성이 완벽해도 의사결정 품질은 보장 못 한다는 게 컴플라이언스 업계의 반복된 교훈. Goodhart의 법칙이 ratchet 축(inject-tokens·mechanism-count)에 그대로 적용된다.

### CA-10: 유지보수 플라이휠이 거버넌스 대상을 잡아먹고 있다 `[execution]` · severity: **high**

Review #1의 CA-3("모양 자체가 무겁다")이 출시 후 데이터로 확증됐다: 4,018 LoC, 문서 270KB(문서:코드 ≈ 67:1), 규칙 95개, 메커니즘 26개 — 사용자 1명. 더 나쁜 신호는 성장의 비대칭이다: 추가는 매 버전 일어나지만 은퇴는 15버전 동안 **단 1건**(#015 kill-check), 그조차 "0회 발화" 증거가 ~10버전 누적된 뒤에야 가능했다. 더하기는 싸고 빼기는 장례식이 필요하다. `version-doctor.py`는 그 자체가 증거다 — vendoring 배포 모델(#014)이 만든 3점 버전 드리프트를 사후에 봉합한 2차 복잡성이고, `session_count`는 kill-check 은퇴 후 측정만 되고 쓰이지 않는다(비용만 남은 메커니즘). #011 entropy-gc가 빼기를 시도했으나 2건 탐지 중 0건만 안전 삭제 가능했다 — 표면 GC조차 relic을 못 가렸다.

**선례**: Brooks의 second-system effect + "생산성 시스템이 곧 미루기"가 되는 정교한 Zettelkasten/Notion 패턴(후자는 부분적으로 speculative — 그러나 git log가 이미 그 방향을 가리킨다).

## 다중 페르소나 공격

Review #1은 페르소나를 "시간을 가로지른 본인"으로 한정했다. v0.3.8은 marketplace 배포물이 됐으므로 **외부 stakeholder가 처음으로 실재**한다 — 페르소나를 확장한다.

### 페르소나 3 — 첫날 신규 사용자 (NEW, 배포 모델의 직접 결과)
설치하자마자 코드 수정이 훅에 차단되고, phase를 진행하려면 evidence 파일과 confirm이 필요하며, 규칙 95개가 세션마다 주입된다. "내 프로젝트를 도와주는 도구"가 아니라 "내가 통과해야 하는 시험"으로 느껴지는 순간 uninstall은 `rm -rf .claude/harness` 한 줄이다. GOAL §3.5(첫 사이클 가치 증명)를 시스템 스스로 가장 위협한다.

### 페르소나 4 — 6개월 후의 작성자 (Review #1 페르소나 1의 v0.3.8판)
새벽에 터지는 건 제품이 아니라 harness다 — 훅 7개와 스크립트 16개의 상호 의존(chainlog↔ratchetlib↔ruleslib)에서 한 곳이 어긋나면 *자기 프로젝트의 모든 Edit이 차단*된다. exit 2 차단 훅은 버그가 곧 작업 정지를 의미한다.

### 페르소나 5 — 대안(경쟁자)
잘 쓴 50줄 CLAUDE.md + phase-guard 훅 1개면 이 시스템 가치의 70-80%가 나온다. 나머지 24개 메커니즘의 한계 가치를 입증하는 데이터가 없다 — "더 단순한 대안 대비 무엇이 나은가"에 현재 답은 "내적 일관성"뿐이다.

## 핵심 취약점 (CV-2)

### CV-2: 검증의 *방향*이 안쪽을 향한다 — compliance를 측정하지 outcome을 측정하지 않는다

CV-1(author=enforcer=target)이 *강제*의 결함이라면, CV-2는 *검증*의 결함이다. 둘은 형제다. 이 시스템은 "속일 수 없는 프로세스"를 만드는 데 모든 정교함을 쏟았지만, 유일하게 중요한 질문 — *이 프로세스를 따르면 더 좋은 제품이 더 빨리 나오는가* — 은 측정조차 설계되어 있지 않다. HA-8이 무기화된 형태다. ratchet은 *일관성 메커니즘*이지 *품질 메커니즘*이 아니다: inject-tokens가 384든 766이든, 그것이 최종 산출물을 개선한다는 증거가 없다. CV-1을 "사람→코드"로 옮긴 것(Resolution log)은 *누가 차단하는가*를 옮긴 것이지 *누가 가치를 판정하는가*를 옮긴 게 아니다 — 후자는 여전히 author 본인이고, 그래서 11사이클 동안 "harness가 작동한다"는 모든 증거가 *harness 자신에 대한* 것이었다(#006~#008 dogfood는 "harness가 자기 자신에 작동함"을 증명했을 뿐, "harness가 제품을 낫게 함"을 증명하지 않았다).

## 가역성 (Reversibility)

**reversible** — 스크립트와 문서일 뿐이고 제거는 즉시 가능(`rm -rf .claude/harness`).

다만 Review #1이 경고한 *시간에 따른 reversibility 감소*가 진행됐다: 11사이클·15버전의 sunk cost + 정체성 애착이 prune을 더 비싸게 만들었다(#011이 빼기에 거의 실패한 게 신호). critical 반론(CA-8)도 "중단하라"가 아니라 **"다음 한 수는 메커니즘 추가가 아니라 외부 검증이어야 한다"**를 뜻한다. 비가역적인 것은 단 하나 — 계속 자기참조 루프에 쓰는 시간이다.

## 개선 경로 (PF-8부터)

### PF-8 (CA-8 해소 조건) — harness가 아닌 *외부 제품*을 1사이클 완주
harness 리포가 아닌 실제 제품 하나를 harness로 끝까지(기획→검증→구현→출시) 1사이클 돌린 데이터가 존재할 것. N=1이라도 *외부 대상*이어야 H1이 처음 측정된다. 가능하면 "harness 없이 돌린 비슷한 작업"과 비교 가능한 신호(사이클 길이, 재작업량, 출시 여부)를 남긴다. **이것 없이 v0.4 기능을 쌓는 것은 자기 문서가 정의한 AP-01(검증 연극)이다.**

### PF-9 (CA-9 해소 조건) — soft 위반 발화 통계로 hard/soft 경계 재조정
`feedback.jsonl` friction log 기반으로 soft 규칙이 실제로 얼마나 자주 깨지는지(어떤 R-CD/R-DD 룰이 무시되는지) 집계한다. 그 데이터로 (a) 자주 깨지면서 결과에 치명적인 룰을 hard hook 후보로 승격, (b) 한 번도 안 깨지거나 깨져도 무해한 룰을 retire 후보로 강등. 강제의 *방향*을 데이터로 교정한다.

### PF-10 (CA-10 해소 조건) — "은퇴 후보" 검토를 사이클 종료 게이트의 정규 항목으로
close-cycle 게이트에 "이번 사이클에 추가한 메커니즘 중 다음 후보가 있는가: 0회 발화 / 측정만 되고 미사용 / canonical 중복"을 *정규 점검 항목*으로 넣어, 빼기가 더하기와 같은 비용이 되게 한다. #015가 ~10버전 걸린 이유는 은퇴가 *예외 이벤트*였기 때문 — 이를 *정기 절차*로 만든다. ratchet에 `mechanism-count`가 *내려갈 때 보상*하는 축(혹은 단조 증가 금지 검토)을 추가.
...
| 순위 | 작업 | 차단 효과 |
|---|---|---|
| 1 | PF-8 외부 제품 1사이클 완주 | CV-2의 *유일한* 해소 경로 — 이게 없으면 모든 후속 기능이 검증 연극 |
| 2 | PF-10 은퇴 게이트 정규화 | 플라이휠 제동 — 늦을수록 sunk cost로 비싸짐 |
| 3 | PF-9 soft 위반 통계 | PF-8 사이클에서 friction 데이터를 수집하며 동시 진행 |
...
- 2026-06-02 — **entropy gc — 원칙6를 우리 코드에 처음 적용 (사이클 #011)** — `gc-scan.py`(결정론적 *표면* 엔트로피 스캐너)+`GOLDEN-PRINCIPLES.md`(선언된 GP-1/2/3, 비해석)+hermetic self-test가 하네스 리포지토리의 죽은 링크·relic 디렉토리·중복 파서를 스캔. ratchet 축 2개(`harness-entropy-found`↑·`harness-entropy-remaining`↓) 등록 → 미래 사이클이 엔트로피 재유입 시 close 게이트(#008)가 차단. **CA-3("압축이 아니라 모양이 무겁다")·사용자 "무겁다"에 대한 첫 응답**: GC는 *표면* 줄이기(측정 먼저), 토큰 압축은 다음 사이클로 분리. 정리: dead link 4개(→`templates/retro.md` 생성)+hooks/README "구현 상태" 역방향 stale 정정(5개 구현 반영). **핵심 발견(F1)**: 구조 휴리스틱(README+코드0+canonical존재)이 *signpost↔relic을 원리적으로 못 가른다* — 2건 탐지 중 **0건이 안전 삭제**(scripts/README=살아있는 가이드, hooks/README=상태만 stale). #009 F1·#010 F1의 3연속("실적용 전엔 맞아 보이는 탐지기") → GP-1을 high→watch 강등, "내용 판단 필요한 건 high-confidence 아니다" 원칙화. **하네스 자기 원칙이 자기 도구의 오탐을 막음**: "삭제 전 대상을 직접 보라"를 지켜 GP-1 high를 안 믿고 내용을 읽어 둘 다 살림. **독립 리뷰(원칙3)가 self-test에 sabotage 3건 주입해 비-vacuous 입증** + low 2건 포착(docstring [high] drift·--root raw 에러) → 즉시 수정. B1~B5 fresh 재검증 전부 PASS, fixpoint exit0. **정직한 한계**: ⓐGC 첫 패스가 얕음 — high 4건이 1 root cause(F4) ⓑ의미적 stale(F2)은 결정론 스캔이 못 잡음, mandatory 사람 내용검토 의존(명문화 backlog) ⓒGP-1 watch의 실용가치 미입증(0/2 정밀도 — 진짜 relic 한 번 잡아야) ⓓWET 파서 GP-3 watch(2멤버, 3번째 등장 시 ESCALATE) ⓔ표면 GC만, 토큰 GC는 다음 키스톤. `cycles/20260602-entropy-gc/` 참조.
