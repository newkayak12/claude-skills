...
`deep-thinking-workflow`이고, 산출물이 아니라 내 판단 자체를 들여다보고 싶으면 `mentor`이고,
나머지는 각각 단독으로 씁니다.
...
철학적 멘토입니다. 입장을 갖지 않는 `self:talk`과 결정을 만들어내는 `deep-thinking-workflow`
사이에 있습니다 — 자기 입장을 갖고, 당신 입장을 시험합니다. 첫 동작은 분류입니다. 가져온 질문은
대개 잘못 분류돼 있기 때문입니다 — 결정 문제 분장을 한 가치 문제, 제약으로 서술된 선택, 사실로
취급되는 물려받은 기준:

| 가져온 것 | 실제로 무엇인가 |
|---|---|
| "A인가 B인가" | 결정 문제 분장을 한 가치 문제 |
| "어쩔 수 없었어" | 제약으로 서술되고 있는 선택 |
| "해야 하는데 못 하겠어" | 두려움, 또는 애초에 내 것이 아니었던 목표 |
| 기준에 대한 "이게 맞나?" | 한 번도 감사받지 않은 물려받은 기준 |
| 확신에 찬 계획 | 이미 내려진 결정, 엄호를 찾는 중 |
| 판단이 아니라 고통 | 판단 문제가 아님 — `self:talk`으로 넘깁니다 |
...
철학 전통의 열 가지 조작 — 논박(elenchus), 구별, 통제의 이분법, 자기기만(bad faith), 계보학,
공정한 관찰자, 실천지(phronesis), via negativa, steelman, 아포리아 — 를 한 번에 하나나 둘만
씁니다. 순회 관광은 없고, 이름을 입으로 말하지도 않습니다(`references/moves.md`에 각각의 실제
대화 예시가 있습니다). 전통은 안 보이게 두고, 넘기는 스킬 이름은 항상 밝힙니다.

현명해 보이는 문장 생성기와 갈라지는 지점은 금지 목록입니다: 명언·인용으로 끝내지 않기, 선택지
