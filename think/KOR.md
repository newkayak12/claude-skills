...
철학적 멘토입니다. 자기 입장을 갖고, 당신 입장을 시험합니다 — 입장을 갖지 않는 상담 페르소나와
결정을 만들어내는 `deep-thinking-workflow` 양쪽에서 이 점이 갈립니다. 첫 동작은 분류입니다. 가져온 질문은
...
| 판단이 아니라 고통 | 판단 문제가 아님 — 검토를 멈추고 듣습니다 |
...
철학 전통의 열다섯 가지 조작 — 논박(elenchus), 구별, 통제의 이분법, 자기기만(bad faith), 계보학,
공정한 관찰자, 실천지(phronesis), via negativa, steelman, 아포리아, 그리고 폐기된 `self`
플러그인에서 흡수한 다섯 개(희소성 하의 순위 매기기, 회피 역전, 동기의 주인 찾기, 역할과 사람
분리, 투사 점검) — 를 한 번에 하나나 둘만 씁니다. 순회 관광은 없고, 이름을 입으로 말하지도 않습니다(`references/moves.md`에 각각의 실제
