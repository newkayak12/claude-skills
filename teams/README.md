...
## Status
- v0.31.5 — **codex implements again, and the integrate judge is told its job again**: (1) the codex adapter's `implement`/`test` output schemas were refused by OpenAI's strict structured outputs (`400 invalid_json_schema`: `upstream_defects` items lacked `additionalProperties: false`, and optional properties were not in `required`) since `upstream_defects` was added — code-beta-X2, the first teams run to route implement to codex (`policy` + `danger-full-access`), failed every codex node "adapter exit 1" before codex did any work. Optional fields are now required-but-nullable, nulls are stripped from the result, and `test-codex-adapter.mjs` checks every object node against the strict rules (it fails on the old schema). (2) 0.30.2 had dropped "Run the goal-level checks there. Read the seams" from the integrate briefing; restored with a test (an audit of every removed line since found no other). (3) At integration the manager runs each merged package's own `npm test --prefix <dir>` and hands the exit codes to the judge as facts (seam-beta-D2: cli 0/10 on its own, never caught). (4) Story pages list what the story implements, its acceptance, and every attempt with the first thing that sank it. (5) The viewer reads a task whose daemon and drivers are all dead as `stalled`, not `running`. (6) A boxed Sprint's planning team is told the box exists (code-sprint-P2 spent ~$14.5 of $15 on four planning documents). (7) `bench.sh`: `BENCH_POLICY` passes a routing policy to `tm_open`.
- v0.31.4 — **codex under `danger-full-access` is reachable again**: 0.31.2 made codex's smoke run a command, but the smoke was hardcoded `-s read-only` — so on this host (Ubuntu 24.04, `kernel.apparmor_restrict_unprivileged_userns=1`, where bubblewrap cannot create its namespaces: "bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted") codex read not ready even for a run that opted into `danger-full-access`, the per-run answer graph shipped in v1.5.7 for exactly this host. The smoke now runs under the sandbox the stage will use. Real CLI: `danger-full-access` → ready, `read-only` → not reachable. Opt in with `tm_open({policy: {implement: {vendor: "codex", sandbox: "danger-full-access"}, ...}})`; reasoning stages stay read-only by design, so on this host they go to claude or self. The host fix is an AppArmor `userns` profile for `/usr/bin/bwrap`.
- v0.31.3 — **the planning path under a box**: code-sprint-P2 (`roles.planning`, $15 box, 0.31.2) is the first planning run whose PLAN converged — every planning gate passed on its first attempt (93, 92, ...), where code-sprint-S2's rejected three rounds at 93-95. It also spent ~$14.5 of the $15 on planning, and the box stopped right after shape with no package run. Two defects on that path: an integrate over zero accepted packages opened and was judged (now: nothing accepted → every pending node but the report is settled), and the goal gate waited on `accept:AUDIT:1`, a node only a dispatch the stopped box forbids could create — pending nodes, none runnable, the daemon waited 25 minutes. `closeStoppedToReport` now treats that as stuck (a ready dispatch counts as stuck once stopped) and runs only after the package sweep has nothing left to do. The live P2 task, unstuck by a daemon restart, closed to its report with all four backlog items carried forward.
- v0.31.2 — **a running session's cost is visible, and codex must prove it can run a command**: (1) a driver session still running has no result event, so it read $0 until it ended — code-sprint-P1's PLAN session showed $0.00 at 42 minutes while it had spent ~$8.83 of a $15 box. `collectDriverCosts` now estimates it from its assistant messages' token use (input 1, cache write 1.25, cache read 0.1, output 5) times the cost-per-unit of this task's own finished sessions of the same kind — measured 5.2-5.7e-6 across dispatch sessions, and a session with its result event removed estimated $1.47 against $1.46 actual — falling back to the measured rate for claude-opus-5-5 when nothing has finished (`uncalibrated`). Status and viewer show it as `(~$x running, estimated)`, and the box counts it. (2) codex's readiness smoke only asked it to say CODEX_READY, so on this host — where the read-only sandbox cannot start (bubblewrap) — it was routed judge stages and a gate rejected at 0 because it could run no command. The smoke now has codex `cat` a file holding a random token; confirmed against the real CLI (`reachable: false`). (3) A non-interactive author is told a decision it discovers while writing is decided by default too, not added as an open question (the PLAN draft minted nine new ones).
- v0.31.1 — **the next Sprint builds on the last one**: teams never merges its integration branch into the project's own branch, so code-sprint-S8's follow-up (`context_from`) opened on `seed` with none of P1/P2's accepted work under it. `retro.json` now names `integration_branch`, `80-report.md` says it is unmerged, and a task opened with `context_from` whose prior integration branch HEAD does not contain gets `base_ref`: package and integration worktrees branch from it (HEAD again once it is merged). `context_from` itself worked on the real run — the follow-up's `requests` were exactly the two unshipped items. Retro noise: nodes superseded by a reshape or newer attempt are no longer listed as failures, and a refusal argued in `blocking`/`gaps`/`problems`/`unowned` reads as its reason. `UPDATE_GOLDEN=1` regenerates the docs goldens.
- v0.31.0 — **what the sweep of earlier real runs left open**: (1) a passing critique's `problems[]` naming a package now reach that package's child context and its accept judge (`critiqueNotesFor`) — idol-beta-ask1's "raise P4 to 10,000" was read by nobody; (2) a person's decision is the run's, not one subgoal's: `openAsk` filters and every briefing's `prior_decisions` carry the whole run's answers, tagged `decided_for` (the fan-club tier question was answered under U1 and asked again, and answered the other way, under U2 and U3); (3) a task blocked short of its report gets an `80-report.md` (state BLOCKED) naming each blocker with its reason and the `tm_retry` call that would move it, plus `retro.json`; (4) a provider usage limit found only in an adapter's event stream (codex's shape) is a fallback to another vendor under either allocation — code-sprint-S2's second PLAN run spent every retry on codex "You've hit your usage limit" exits; (5) a non-interactive run hands its `unasked` questions to draft/revise/implement as **decided by default** (the recommended option, marked "default - revisit") — code-sprint-S2's PLAN gate rejected three rounds at 93-95 because the documents kept those questions open while the validation matrix asserted answers to them.
- v0.30.3 — **the goal gate of a stopped Sprint**: code-sprint-S6 (budget $5) shipped three of four packages, reintegrated them (`integrate:3` passed), and then the goal gate — `accept: true` at 72 — failed on the whole-goal floor of 90 and the task ended blocked with no report. Two fixes: after a sweep that skipped packages the goal gate's floor is waived (`goal_floor_waived` records why; a refusal is still a refusal), and the daemon runs `enforceBudget` once more before `daemon_done`, since a node that fails at the end of a step left the task blocked before the closer at the top of the next step could see it.
- v0.30.2 — **a stopped Sprint closes to its review**: code-sprint-S5 (budget $5) stopped exactly where it should — 108% after P2 accepted, P3/P4 swept, `integrate:2` over P1+P2 — and then ended `blocked` with no report. Four fixes: (1) 0.30.0's "refusal with no reason" check read only reason/gaps, so integrate's own `unowned`/`evidence` argument read as unexplained — every contract field that says why now counts (a regression of mine, found by the run); (2) the daemon recorded `daemon_done` while a re-judge was still scheduled (`pendingRejudgeAt` — it now waits); (3) the post-stop integrate and goal gate are told the skipped packages are out of scope by design, and judge the kept set; (4) once stopped, no repair or reshape opens (as retries already did not), and a graph blocked short of its report settles every pending node but the report (`budget_closed`), so the Sprint always ends in `80-report.md` and `retro.json`. Also: code-sprint-S4 (budget $10, the 0.30.1 probe routing execute stages to self) delivered all four packages first try, 9/9, $8.88, 21 min.
