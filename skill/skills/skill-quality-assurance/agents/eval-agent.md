...
If `scenarios:` is missing or has fewer than 2 items, derive prompts from the description's trigger phrases. If none available, report `SKIP — no scenarios to test` and stop.
...
2. **What Claude Does** table or **Standing Mandates** — behavioral promises
...
- **Discriminating assertion**: with-skill passes, without-skill fails — this is what the skill is actually enforcing
- **Skill gap**: both configurations fail — the skill promises this but doesn't deliver it
- **Redundant coverage**: without-skill passes just as well — the skill may not be earning its place here
