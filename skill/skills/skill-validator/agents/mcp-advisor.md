...
You are evaluating whether a skill could benefit from MCP (Model Context Protocol) tools — specifically the three that are commonly available: `sequential-thinking`, `think-tool`, and `mcp-reasoner`.
...
- Produce output that must be built up incrementally (each step depends on the previous)
- Avoid jumping to conclusions by forcing structured progression

Not helpful when: the task is straightforward, doesn't require multi-step reasoning, or the output is a single lookup / retrieval.
...
- Work through ambiguity without committing to an answer prematurely
- Do the equivalent of "thinking out loud" before producing output
...
- Produce rigorously argued outputs (like architecture decisions, complex debugging)

Not helpful when: the problem has a single correct answer that follows from the inputs, or when speed matters more than thoroughness.
...
- Why it helps (what problem it solves in this skill's context)
...
[If applicable: specific suggestion, e.g., "Add think-tool call before the architecture decision step at line X — the model currently jumps straight to output without weighing tradeoffs."]
```
