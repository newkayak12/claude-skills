...
def _governance_block() -> str:
    ...
    return (
        ...
        "이 디렉토리에는 `.claude/harness/`(벤더링된 하네스)가 설치돼 있다. 작업할 때 다음을 *기본*으로 따른다 —\n\n"
        "- **작업 = 사이클.** 새 기능·구조 변경은 사이클로 연다: `python3 .claude/harness/scripts/cycle-init.py \"<name>\" --type dev-tool`.\n"
        "- **WIP=1.** 동시에 *열린* 사이클은 하나. 새로 열기 전에 진행 중인 것을 닫는다.\n"
        "- **게이트(품질 저하 방지 3층).** 사이클은 ①가설/품질-바를 *먼저* tamper-evident 로 잠그고 "
        "(`hypothesis-register.py`·`bar-register.py`) → ②독립 리뷰어(doer≠reviewer)가 각 바를 채점 "
        "(`review-register.py`) → ③`close-cycle.py` 게이트로 닫는다. cross-cycle ratchet 이 사이클을 넘어 바가 낮아지는 걸 막는다.\n"
        "- **plan-before-code (스킬 아닌 hook 강제).** 코드는 phase 가 implementation/validation 일 때만 쓴다 — 그 전에 "
        "analysis→design→planning 을 거쳐 산출물(evidence)을 파일로 남기고 `phase-advance.py` 로 전진한다. "
        "PreToolUse phase-guard 가 이를 *자동* 강제하므로(미충족 시 코드 편집 차단) 별도 스킬 호출·AI 재량 없이 적용된다.\n"
        "- **hooks 자동 발화.** `.claude/settings.json` 에 등록된 하네스 hook(SessionStart 룰 주입, "
        "PreToolUse phase-guard/immutability 등)이 매 세션·편집마다 자동 동작한다 — 별도 호출 불필요.\n\n"
        "> 단순 버그픽스·유지보수는 over-engineering 이니 사이클을 강제하지 않는다(GOAL: 솔로 dev 제품 *한 사이클*에 특화).\n"
        f"{CLAUDE_MD_END}"
    )
