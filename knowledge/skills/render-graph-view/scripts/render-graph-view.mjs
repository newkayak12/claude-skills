...
const nodes=data.nodes.map(function(n,index){const seed=hash(n.id),angle=(seed%6283)/1000,radius=70+((seed>>>8)%310);const item=Object.assign({},n,{x:Math.cos(angle)*radius,y:Math.sin(angle)*radius,vx:0,vy:0,index:index,visible:true});nodeMap.set(item.id,item);degree.set(item.id,0);return item});
...
function graphPoint(sx,sy){return{x:(sx-panX)/zoom,y:(sy-panY)/zoom}}
function screenPoint(n){return{x:n.x*zoom+panX,y:n.y*zoom+panY}}
...
function simulate(){if(!running||alpha<.002)return;const cellSize=90,cells=new Map();nodes.forEach(function(n){if(!n.visible||n===dragNode)return;const key=Math.floor(n.x/cellSize)+','+Math.floor(n.y/cellSize);if(!cells.has(key))cells.set(key,[]);cells.get(key).push(n)});
  nodes.forEach(function(n){if(!n.visible||n===dragNode)return;const cx=Math.floor(n.x/cellSize),cy=Math.floor(n.y/cellSize);for(let gx=cx-1;gx<=cx+1;gx++)for(let gy=cy-1;gy<=cy+1;gy++){const group=cells.get(gx+','+gy)||[];group.forEach(function(m){if(m.index<=n.index)return;let dx=n.x-m.x,dy=n.y-m.y,d2=dx*dx+dy*dy+.5;if(d2>15000)return;const f=Math.min(2.8,950/d2)*alpha,dist=Math.sqrt(d2);dx/=dist;dy/=dist;n.vx+=dx*f;n.vy+=dy*f;m.vx-=dx*f;m.vy-=dy*f})}n.vx+=-n.x*.0007*alpha;n.vy+=-n.y*.0007*alpha});
  edges.forEach(function(e){if(!visibleEdge(e))return;let dx=e.b.x-e.a.x,dy=e.b.y-e.a.y,dist=Math.sqrt(dx*dx+dy*dy)||1,f=(dist-72)*.0025*alpha;dx/=dist;dy/=dist;if(e.a!==dragNode){e.a.vx+=dx*f;e.a.vy+=dy*f}if(e.b!==dragNode){e.b.vx-=dx*f;e.b.vy-=dy*f}});
...
function draw(){ctx.clearRect(0,0,width,height);const q=search.value.trim();edges.forEach(function(e){if(!visibleEdge(e))return;const a=screenPoint(e.a),b=screenPoint(e.b),focus=selected&&(e.a===selected||e.b===selected);ctx.strokeStyle=focus?'rgba(190,174,245,.72)':'rgba(132,132,150,'+(selected?'.08':'.24')+')';ctx.lineWidth=focus?1.5:.75;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();if(focus)drawArrow(a,Object.assign({radius:e.b.radius},b),'rgba(190,174,245,.72)')});
  nodes.forEach(function(n){if(!n.visible)return;const p=screenPoint(n),match=matches(n),neighbor=selectedNeighbor(n),focus=n===selected||n===hovered||neighbor;let opacity=1;if(selected&&!focus)opacity=.18;else if(q&&!match)opacity=.13;ctx.globalAlpha=opacity;const r=Math.max(2.4,n.radius*zoom);ctx.fillStyle=colors.get(n.label);if(n===selected){ctx.shadowColor=colors.get(n.label);ctx.shadowBlur=18;ctx.strokeStyle='#f4efff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(p.x,p.y,r+3,0,Math.PI*2);ctx.stroke()}ctx.beginPath();ctx.arc(p.x,p.y,r,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;
    const showLabel=n===selected||n===hovered||(q&&match)||(zoom>.92&&degree.get(n.id)>2);if(showLabel){ctx.globalAlpha=Math.max(opacity,.78);ctx.font=(n===selected?'600 ':'400 ')+Math.max(10,Math.min(14,11*zoom))+'px system-ui';ctx.fillStyle='#e8e8ee';ctx.textAlign='center';ctx.textBaseline='top';ctx.fillText(n.name,p.x,p.y+r+5)}ctx.globalAlpha=1})}
...
canvas.addEventListener('wheel',function(event){event.preventDefault();const rect=canvas.getBoundingClientRect(),sx=event.clientX-rect.left,sy=event.clientY-rect.top,before=graphPoint(sx,sy),factor=Math.exp(-event.deltaY*.0012);zoom=Math.max(.12,Math.min(5,zoom*factor));panX=sx-before.x*zoom;panY=sy-before.y*zoom;draw()},{passive:false});
function fit(){const visible=nodes.filter(function(n){return n.visible});if(!visible.length)return;const xs=visible.map(function(n){return n.x}),ys=visible.map(function(n){return n.y}),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys),margin=90;zoom=Math.max(.12,Math.min(2.2,Math.min((width-margin*2)/(maxX-minX||1),(height-margin*2)/(maxY-minY||1))));panX=width/2-(minX+maxX)/2*zoom;panY=height/2-(minY+maxY)/2*zoom;draw()}
